...
def prepare_data(data):
    ...
    # One-hot encoder on the type and City columns
    data['type'].replace({'דירת גן': 'בית', 'קוטג טורי': 'בית', 'פרטי': 'בית', 'קוטג': 'בית', 'דופלקס': 'בית', 'טריפלקס': 'בית', 'דו משפחתי': 'בית', 'בית פרטי': 'בית', "קוטג'": 'בית', "קוטג' טורי": 'בית', 'נחלה': 'בית', 'מיני פנטהאוז': 'פנטהאוז', 'דירת גג': 'פנטהאוז', 'בניין': 'אחר', 'מגרש': 'אחר'}, inplace=True)
    data['type'] = data['type'].apply(lambda x: 1 if x in ['בית', 'פנטהאוז'] else 0)
    one_hot_encoded = pd.get_dummies(data['type'], prefix='type')
    data = pd.concat([data, one_hot_encoded], axis=1)

    data['City'].replace({'כפר סבא': 'מרכז', 'יהוד מונוסון': 'מרכז', 'רמת גן': 'מרכז', 'בת ים': 'מרכז', 'הרצליה': 'מרכז', 'ראשון לציון': 'מרכז', 'תל אביב': 'מרכז', 'הוד השרון': 'מרכז', 'מודיעין מכבים רעות': 'מרכז', 'חולון': 'מרכז', 'נס ציונה': 'מרכז', 'פתח תקווה': 'מרכז', 'ירושלים': 'מרכז', 'רחובות': 'מרכז', 'חיפה': 'צפון', 'קרית ביאליק': 'צפון', 'בית שאן': 'צפון', 'נוף הגליל': 'צפון', 'צפת': 'צפון', 'נהריה': 'צפון', 'זכרון יעקב': 'צפון', 'נתניה': 'צפון', ' נהריה': 'צפון', 'אילת': 'דרום','רעננה': 'מרכז', 'באר שבע': 'דרום', 'דימונה': 'דרום'}, inplace=True)
    one_hot_encoded_city = pd.get_dummies(data['City'], prefix='City')
    data = pd.concat([data, one_hot_encoded_city], axis=1)
    data['City'] = data['City'].apply(lambda x: 1 if x in ['מרכז'] else 0)

    columns = ['room_number', 'Area','type','City', 'price', 'hasElevator ', 'hasParking ', 'hasStorage ', 'hasAirCondition ', 'hasBalcony ', 'hasMamad ', 'handicapFriendly ']
    return data[columns]